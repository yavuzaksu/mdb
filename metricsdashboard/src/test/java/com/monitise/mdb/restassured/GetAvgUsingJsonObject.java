...
public class GetAvgUsingJsonObject {
	...
	public void getProjects() throws JSONException {
		...
		// System.out.println(jsonPath.getJsonObject("msr.val"));
		// System.out.println(jsonPath.getList("id"));
		// System.out.println(jsonPath.getList("name"));
		int numOfProjects = 0;
		JsonParser parser = new JsonParser();
		/*Parse the string response into json array
		 * Iterate through json array to access internal values
		 * Extract the jsonarray from json object for coverage % which is inside "msr"
		 * Iterate through jsonarray and get internal values for coverage as double
		 * Calculate total coverage for all projects
		 * Calculate avg coverage using totalcoverage/number of projects
		 */
